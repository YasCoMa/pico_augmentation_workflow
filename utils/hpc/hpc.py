...
class HPC():
    ...
    def create_submit_job(self, job_name, n_jobs, force_array = False, job_path = None, elements=[], custom_elements=[], template_script=None, command=None, config=None, add_cnf='', wait=True, ncpus = 1, nmem=15, execpy='python', flag_ollama=False):
        ...
        package = os.environ['PYTHONPATH']
        
        if( job_path is None ):
            tmp_folder = config.PATH.TMP
            job_path = tempfile.mkdtemp( prefix = job_name+'_', dir = tmp_folder )
            
        if( template_script is not None ):
            if( not os.path.isdir(job_path) ):
                os.makedirs(job_path)
            script_path = execpy + ' ' + os.path.join(job_path, 'exec.py')
            f = open( os.path.join(job_path, 'exec.py'), 'w')
            f.write(template_script)
            f.close()

        memv = nmem
        addTask = "<TASK_ID> <FILE>"
        if( n_jobs == 1 and (not force_array)  ):
            memv = nmem
            addTask = ""
...
nohup singularity run --nv  /aloy/scratch/ymartins/ollama/ollama.sif &
        ...
        command = "SINGULARITYENV_PERL5LIB={} SINGULARITYENV_PYTHONPATH={} SINGULARITYENV_UPDATE_CONFIG={} singularity exec {} {} {} "+addTask
        command = command.format(
            perl5, package, config.config_path, add_cnf, singularity_image, script_path )
        # submit jobs
    ...
    def monitor_background_job(self, job_path, job_name, job_id):
        ...
        return self.hpc.monitor_background_job(job_path, job_name, job_id)
    ...
    def test_job(cls, job_path, config=None, params=None):
        ...
        singularity_image = config.PATH.SINGULARITY_IMAGE
        hpc_path = os.path.dirname( os.path.abspath(__file__))

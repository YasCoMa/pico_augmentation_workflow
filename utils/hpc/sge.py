...
export SINGULARITY_BINDPATH="/aloy/home,/aloy/data,/aloy/web_repository,/aloy/scratch"
        ...
        if (self.nodelist != None and self.nodelist != ''):
            nodes = ','.join( [ 'all.q@'+str(n) for n in self.nodelist] )
            self.queue = nodes
...
""".format(nodes)
    ...
    def submitMultiJob(self, command, **kwargs):
        ...
        membycore = int(kwargs.get("mem_by_core", 15))
        ...
        try:
            ...
            self.statusFile = os.path.join(self.jobdir, self.job_name + sge.jobStatusSuffix)
